...
app.post("/api/analyze", async (req, res) => {
  ...
  try {
    ...
    const prompt = `
      You are StockPilot AI, a premier financial analyst. Analyze the following stock:
      - Stock Name: ${name}
      ...
      Strict guidelines:
      1. Provide authentic, financially sound reasons specific to ${name}'s current market situation.
...
app.post("/api/news", async (req, res) => {
  ...
  try {
    ...
    const prompt = `
      You are StockPilot AI, a financial editor. Summarize and analyze the latest market trends for ${name} (${symbol}).
      
      Here are some mock/real news headlines or contexts for this stock:
      ${JSON.stringify(headlines || [])}

      Generate 3 highly realistic and detailed news summaries for ${name}. Return a JSON object matching this schema:
      {
        "news": [
          {
            "id": "news_1",
            "title": "A highly professional Korean financial news headline for ${name}",
            "source": "E.g., 연합인포맥스, 한국경제, Bloomberg, etc.",
            "time": "E.g., 30분 전, 1시간 전, 4시간 전",
            "sentiment": "bullish" or "bearish" or "neutral",
            "summary": "A cohesive 2-sentence summary in Korean highlighting the market implications of this news."
          },
